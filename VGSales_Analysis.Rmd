...
WIIGenre <- datFrame %>%
  group_by(Genre) %>%
  summarise(total = sum(Platform == "Wii", na.rm = T))
pie(WIIGenre$total, labels = WIIGenre$Genre)

DSGenre <- datFrame %>%
  group_by(Genre) %>%
  summarise(total = sum(Platform == "DS", na.rm = T))
pie(DSGenre$total, labels = DSGenre$Genre)

PS2Genre <- datFrame %>%
  group_by(Genre) %>%
  summarise(total = sum(Platform == "PS2", na.rm = T))
pie(PS2Genre$total, labels = PS2Genre$Genre)

X360Genre <- datFrame %>%
  group_by(Genre) %>%
  summarise(total = sum(Platform == "X360", na.rm = T))
pie(X360Genre$total, labels = X360Genre$Genre)

PCGenre <- datFrame %>%
  group_by(Genre) %>%
  summarise(total = sum(Platform == "PC", na.rm = T))
pie(PCGenre$total, labels = PCGenre$Genre)
